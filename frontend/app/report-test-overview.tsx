...
export default function ReportTestOverviewScreen() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const url = `${backendHost}/report/test?course=${encodeURIComponent(course)}&employeeType=${encodeURIComponent(employeeType)}&startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}`;
      console.log('[TestOverview] Fetching:', url);
      const res = await fetch(url);
      const data = await res.json();
      console.log('[TestOverview] Response:', JSON.stringify(data));
      ...
      console.log('[TestOverview] Error:', err);
      setReportResults([]);
      setChartData([]);
    } finally {
      setIsLoading(false);
