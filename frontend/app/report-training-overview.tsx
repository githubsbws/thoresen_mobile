...
export default function ReportTrainingOverviewScreen() {
  ...
  const [fromYear, setFromYear] = useState(t('fromYear'));
  const [toYear, setToYear] = useState(t('toYear'));
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const url = `${backendHost}/report/training?course=${encodeURIComponent(course)}&employeeType=${encodeURIComponent(employeeType)}&startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}`;
      console.log('[TrainingOverview] Fetching:', url);
      const res = await fetch(url);
      const data = await res.json();
      console.log('[TrainingOverview] Response:', JSON.stringify(data));
      ...
      console.log('[TrainingOverview] Error:', err);
      setReportResults([]);
      setChartData([]);
    } finally {
      setIsLoading(false);
      ...
      <View style={styles.container}>
        ...
        >
          ...
          <View style={styles.searchBox}>
            ...
            <View style={styles.formBody}>
              ...
              <CustomSelect
                ...
                options={[t('selectPosition'), ...positionOptionsBase]}
                maxHeight={330}
              ...
              <CustomSelect
                ...
                options={[t('fromYear'), '2018', '2019', '2020', '2021', '2022', '2023', '2024']}
              ...
              <CustomSelect
                ...
                options={[t('toYear'), '2018', '2019', '2020', '2021', '2022', '2023', '2024']}
          ...
          <View style={styles.resultBox}>
            <Text style={styles.resultTitle}>{t('overviewTrainingCourseReport')}</Text>
            {isLoading ? (
                ...
                {reportResults.length === 0 ? (
                  ...
                  reportResults.map((item: any, idx: number) => (
                    <View key={idx} style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#EEF2F6' }}>
                      <Text style={{ fontWeight: 'bold', fontSize: 16, color: PRIMARY }}>{item.title || item.name || `Training #${item.id}`}</Text>
                      <Text style={{ color: '#555', marginTop: 2 }}>Start Date: {item.startDate || 'N/A'} | Status: {item.status}</Text>
                      <Text style={{ color: 'green', fontSize: 12, marginTop: 4 }}>Completion Rate: {item.rate || item.completionRate || '100%'}</Text>
                    </View>
                  ))
                )}
              </>
        ...
        <Modal visible={menuVisible} transparent animationType="fade" onRequestClose={() => setMenuVisible(false)}>
          ...
          <View style={styles.menuOverlay}>
            ...
            <View style={styles.menuBox}>
              ...
              {menuList.map((item) => (
                <TouchableOpacity key={item.route} style={styles.menuItem} onPress={() => goMenu(item.route)}>
...
const styles = StyleSheet.create({
  ...
  checkboxText: { fontSize: 20, fontWeight: '700', color: '#333', marginRight: 18 },
  dateInput: {
    height: 46,
    borderWidth: 1,
    borderColor: '#D1D5DB',
