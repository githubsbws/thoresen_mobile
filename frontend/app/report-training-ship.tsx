...
export default function ReportTrainingShipScreen() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const url = `${backendHost}/report/training?course=${encodeURIComponent(course)}&department=${encodeURIComponent(department)}&position=${encodeURIComponent(position)}&startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}`;
      console.log('[TrainingShip] Fetching:', url);
      const res = await fetch(url);
      const data = await res.json();
      console.log('[TrainingShip] Response:', JSON.stringify(data));
      ...
      console.log('[TrainingShip] Error:', err);
      setReportResults([]);
      setChartData([]);
    } finally {
      setIsLoading(false);
      ...
      <View style={styles.container}>
        ...
        >
          ...
          <View style={styles.resultBox}>
            <Text style={styles.resultTitle}>{t('trainingCourseShipStaff')}</Text>
            {isLoading ? (
              ...
              <>
                {reportResults.length > 0 && (
                  <ReportChart
                    type={chartType}
                    title="Ship Staff Training Chart"
                    data={
                      chartData.length > 0
                        ? chartData
