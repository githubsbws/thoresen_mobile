...
export default function ReportOverviewScreen() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const url = `${backendHost}/report/search?employeeType=${encodeURIComponent(employeeType)}&department=${encodeURIComponent(department)}&position=${encodeURIComponent(position)}&status=${encodeURIComponent(status)}&startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}`;
      console.log('[ReportOverview] Fetching:', url);
      const res = await fetch(url);
      const data = await res.json();
      console.log('[ReportOverview] Response:', JSON.stringify(data));
      ...
      console.log('[ReportOverview] Error:', err);
      setReportResults([]);
      setChartData([]);
    } finally {
      setIsLoading(false);
      ...
      <View style={styles.container}>
        ...
        >
          ...
          <View style={styles.searchBox}>
            ...
            <View style={styles.formBody}>
              ...
              <CustomSelect
                ...
                options={[t('selectDepartment'), 'Marine', 'HR', 'MPD', 'MTD', 'QAS', 'Technical']}
          ...
          <View style={styles.resultBox}>
            <Text style={styles.resultTitle}>{t('overviewRegisterReport')}</Text>
            {isLoading ? (
              ...
              <>
                {reportResults.length > 0 && (
                  <ReportChart
                    type={chartType}
                    title="Overview Registration Chart"
                    data={
                      chartData.length > 0
                        ? chartData
                        : reportResults.slice(0, 6).map((item: any, idx: number) => ({
                            label: item.dept || item.department || item.division || `Dept ${idx + 1}`,
                            value: 1,
                          }))
                    }
                  />
                ...
                {reportResults.length === 0 ? (
                  ...
                  reportResults.map((item: any, idx: number) => (
                    <View key={idx} style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#EEF2F6' }}>
                      <Text style={{ fontWeight: 'bold', fontSize: 16, color: PRIMARY }}>{item.name || item.title || `User #${item.id}`}</Text>
                      <Text style={{ color: '#555', marginTop: 2 }}>Department: {item.dept || item.department || 'General'} | Position: {item.pos || item.position || 'Staff'}</Text>
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={styles.menuOverlay}>
        ...
        <View style={styles.menuBox}>
          ...
          {menuList.map((item) => (
            <TouchableOpacity key={item.route} style={styles.menuItem} onPress={() => onPress(item.route)}>
