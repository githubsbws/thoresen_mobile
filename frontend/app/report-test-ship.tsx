...
type SelectKey =
  | 'course' | 'gen' | 'department' | 'position'
...
const departments = ['DECK DEPARTMENT', 'ENGINE DEPARTMENT'];
...
const positions = [
  ...
  'OILER', 'E-FITTER', 'E-BOY', 'E-CADET',
];

export default function ReportTestShipScreen() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const url = `${backendHost}/report/test?course=${encodeURIComponent(course)}&department=${encodeURIComponent(department)}&position=${encodeURIComponent(position)}&startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}`;
      console.log('[TestShip] Fetching:', url);
      const res = await fetch(url);
      const data = await res.json();
      console.log('[TestShip] Response:', JSON.stringify(data));
      ...
      console.log('[TestShip] Error:', err);
      setReportResults([]);
      setChartData([]);
    } finally {
      setIsLoading(false);
      ...
      <View style={styles.container}>
        ...
        <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.resultBox}>
            <Text style={styles.resultTitle}>{t('testResultShipStaff')}</Text>
            {isLoading ? (
              ...
              <>
                {reportResults.length > 0 && (
                  <ReportChart
                    type={chartType}
                    title="Ship Staff Test Results"
                    data={
                      chartData.length > 0
                        ? chartData
                        : reportResults.slice(0, 8).map((item: any, idx: number) => ({
                            label: item.course ? String(item.course).split(' ').slice(0, 2).join(' ') : `Test ${idx + 1}`,
                            value: item.numericScore ?? (parseFloat(String(item.score || '0').split('/')[0]) || 0),
                          }))
                    }
                  />
                )}
                {reportResults.length === 0 ? (
                  ...
                  reportResults.map((item: any, idx: number) => (
                    <View key={idx} style={{ paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#eee' }}>
                      <Text style={{ fontWeight: 'bold', fontSize: 16, color: PRIMARY }}>{item.course || `Test #${item.id}`}</Text>
                      <Text style={{ color: '#555', marginTop: 2 }}>Candidate: {item.userName || `User #${item.userId}`} | Date: {item.date || 'N/A'}</Text>
                      <Text style={{ color: item.status === 'PASS' ? 'green' : 'red', fontWeight: '600', fontSize: 13, marginTop: 4 }}>Score: {item.score} ({item.scorePercentage || item.numericScore}%) | Status: {item.status}</Text>
                    </View>
                  ))
                )}
              </>
